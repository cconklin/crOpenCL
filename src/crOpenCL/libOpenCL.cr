module CrOpenCL
  ...
  lib LibOpenCL
    ...
    type Kernel = Void*
    type PlatformID = Void*
    type DeviceID = Void*
    type Context = Void*
    type CommandQueue = Void*
    type Mem = Void*
    type Program = Void*
    type Event = Void*
    type Sampler = Void*
    ...
    fun clGetProgramBuildInfo(program : Program, device : DeviceID, param_name : UInt64, param_value_size : UInt64, param_value : Void*, param_value_size_ret : UInt64*) : Int32
    fun clReleaseProgram(program : Program) : Int32

    # Contexts
    # FIXME: properties is actually a cl_context_properties enum *
    fun clCreateContext(properties : UInt64*, num_devices : UInt32, devices : DeviceID*, pfn_notify : (UInt8*, Void*, UInt64, Void* -> Void), user_data : Void*, errcode_ret : Int32*) : Context
    fun clReleaseContext(context : Context) : Int32

    # Command Queues
    # FIXME: properties is actually a cl_command_queue_properties enum
    fun clCreateCommandQueue(context : Context, device : DeviceID, properties : UInt64, errcode_ret : Int32*) : CommandQueue
    ...
    fun clGetKernelWorkGroupInfo(kernel : Kernel, device : DeviceID, param_name : UInt64, param_value_size : UInt64, param_value : Void*, param_value_size_ret : UInt64*) : Int32
    ...
    fun clEnqueueWriteBuffer(command_queue : CommandQueue, buffer : Mem, blocking_write : UInt64, offset : UInt64, cb : UInt64, ptr : Void*,
                             num_events_in_wait_list : UInt32, event_wait_list : Event*, event : Event) : Int32
    # FIXME: blocking_read is a cl_bool enum
    fun clEnqueueReadBuffer(command_queue : CommandQueue, buffer : Mem, blocking_read : UInt64, offset : UInt64, cb : UInt64, ptr : Void*,
                            num_events_in_wait_list : UInt32, event_wait_list : Event*, event : Event) : Int32
    fun clReleaseMemObject(memobj : Mem) : Int32

    # Events
    # FIXME: param_name is actually a cl_profiling_info enum
    fun clGetEventProfilingInfo(event : Event, param_name : UInt64, param_value_size : UInt64, param_value : Void*, param_value_size_ret : UInt64*) : Int32

    # Device
    # FIXME: device_type is a cl_device_type enum
    fun clGetDeviceIDs(platform : PlatformID, device_type : UInt64, num_entries : Int32, devices : DeviceID*, num_devices : UInt32*) : Int32
